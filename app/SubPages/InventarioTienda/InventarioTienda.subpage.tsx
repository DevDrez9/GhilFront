...
const InventarioTienda = () => {
  ...
  const handleDelete = async (id: number) => {
    // Mantenemos la confirmación nativa para la decisión inicial
 ...
 const handleCloseEdit = () => {
  ...
  const imageUrl = imagePath 
    ...
    : null; // Si no hay ruta, imageUrl es null
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Contenedor para toda la información (detalles y acciones) */}
          ...
          >
            <div style={{ flex: 1 }}>
              ...
              <div
                style={{
                  display: "grid",
                  gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", // Ajustado para mejor layout
                  gap: "8px",
                }}
              >
                <div>
    ...
    <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginTop: '5px' }}>
        {item.stock && Object.keys(item.stock).length > 0 ? (
            Object.entries(item.stock).map(([talla, cantidad]) => (
                ...
                >
                    {talla}: <strong>{cantidad as number}</strong>
                </span>
            ))
        ) : (
