...
const InventarioSucursal = () => {
  ...
  const {
    ...
    isError,
    error,
    deleteInventarioSucursal,
    isDeleting,
  } = useInventarioSucursal(debouncedSearch);
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("¿Estás seguro de eliminar este item de inventario?")) {
      try {
        await deleteInventarioSucursal(id);
      ...
      <div className="cuerpoInventarioSucursal">
        ...
        {inventario.length === 0 && (
          ...
          >
            {debouncedSearch ? (
              <>
                <h3>No se encontraron items en el inventario de sucursal</h3>
